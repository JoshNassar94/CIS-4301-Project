...
query = "Create table Activity(ActivityID integer not null, FacilityID integer, AcitivtyDate date, primary key(ActivityID), foreign key(FacilityID) references Facility(FacilityID))"
SQL.write(query + ";\n")
...
query = "Create table CheckIn(CheckInID integer not null, PersonID integer, CheckInDate date, FacilityID integer, ActivityID integer, primary key(CheckInID), foreign key(PersonID) references Person(PersonID), foreign key(FacilityID) references Facility(FacilityID), foreign key(ActivityID) references Activity(ActivityID))"
SQL.write(query + ";\n")
...
query = "Create table EquipmentTransaction(TransactionID integer not null, EquipmentID integer, PersonID integer, TransactionDate date, TransactionType varchar(1), primary key(TransactionID), foreign key(EquipmentID) references Equipment(EquipmentID), foreign key(PersonID) references Person(PersonID))"
SQL.write(query + ";\n")
